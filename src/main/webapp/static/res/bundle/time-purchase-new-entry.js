...
/***/ function(module, exports, __webpack_require__) {
	...
	$(function(){
		...
		var saveTime = 0;
		var headTipsCookie = window.localStorage.getItem("count");
		if(headTipsCookie != 1){
			$(".head-tips").show();
		};
		window.localStorage.setItem("count", 1); // 只有第一次打开才会有提示信息
		...
		function limitNumberRule() {
			...
			$('#selected-goods').delegate('.limit-price', 'keyup', function(){
				...
				if(currPrice.split('.').length == 2){
					var countNumber = currPrice.split('.')[1];
		...
		function bindEvent() {
			$('#isOnSale').change(function(){
				goodsFilter.item_status = goodsFilter.item_status === '4' ? "" : '4';
				getGoodsList();
			});
			...
			$('#seach-bth').click(function(){
				goods_count = 1;
				goodsFilter.key = $('#keyword-input').val();
			...
			$('#save-selected-btn').click(function(){
				...
				if(campaignInfo.goods_info.length != 0) {
					if(saveTime == 0){
						...
						$.post(api_path + '/marketing/limtedPurchase/addLimtedPurchase.do', parm, function(msg){
							if(msg.code == 10000){
								activeId = msg.data;
								layer.msg('保存成功');
