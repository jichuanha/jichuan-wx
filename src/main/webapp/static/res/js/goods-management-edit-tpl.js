var self_tpl = {
	...
	'editGoodsTpl': '{{~ it:item }}\
						<tr class="parent-tr" sku-id="{{= item.item_sku_id }}"  spu-id="{{= item.item_id}}" item_sku_sn="{{= item.item_sku_sn}}" sellerId="{{= item.sellerId}}">\
							<td>\
								<div class="name">\
									<img src="{{= item.img }}">\
									<p>{{= item.item_name }}</p>\
							...
							<td class="price">\
								<p>现价：￥<span>{{? item.price }}{{= item.price}}{{??}}-{{?}}</span></p>\
								<p>原价：￥<span>{{? item.market_price }}{{= item.market_price}}{{??}}-{{?}}</span></p>\
							</td>\
							<td class="num">\
								<p>{{? item.sku_code }}{{= item.sku_code}}{{??}}-{{?}}</p>\
							</td>\
							<td><i class="icon-remove icon-large delete-icon"></i></td>\
						</tr>\
					{{~}}',
