$(function(){
	...
	function getLable() {
		$.get('/getGoodsLable.do', 
			...
			function(data){
				$('#mark-lable-list').html(doT.template(self_tpl.lableTpl)(data));
				pageCtrl($('#label-list-pager'), goodsFilter.current_page, Math.ceil(data.data.total_count/iconFilter.page_size));
			}
		);
	};
	...
	// 收集数据
		function collectData(){
			...
			$('.per-session tbody tr').each(function(){
				 var item = {};
				item.item_id = $(this).attr('spu-id');
				item.img = $(this).find(".name img").eq(0).attr("src");
				item.item_name = $(this).find(".name p").eq(0).text();
				item.item_sku_sn = $(this).attr('item_sku_sn');
				item.sku_code = $(this).find(".num p").eq(0).text();
				item.seller_id = $(this).attr("sellerid");
				item.price = $(this).find(".price p span").eq(0).text();
				item.market_price = $(this).find(".price p span").eq(1).text();
				item.item_sku_id = $(this).attr('sku-id');
				campaignInfo.band_goods.push(item);
			});
			return campaignInfo;
		}
	// 检查表单是否为空
	...
	function init(){
		$.get('/getGoodsBrand.do', 
			function(data){
				for(var i = 0; i<data.data.data.length; i++){
					$(".brand select").append('<option>'+ data.data.data[i].brand_name +'</option>');
					$(".brand select option").eq(-1).attr("value", data.data.data[i].brand_name);
				};
				getGoodsDdata();
			}
		);
	...
	init();
	function getGoodsDdata(){
		$.get('/getGoodsEdit.do',
			{
				item_id:0
			},
			function(data){
				setGoodsData(data.data.data);
			}
		);
	};
	function setGoodsData(data){
		$('.whole .content .cateName span').eq(0).text(data.cate_name);
		$('.form-horizontal .goodName').eq(0).val(data.good_name);
		$('.form-horizontal .sale-info').eq(0).val(data.sale_info);
		$(".brand select").eq(0).val(data.brand_name);
		$(".add-icon-btn").html('<img src="' + data.icon_img.link + '" class="img-circle" data-iconID="' + data.icon_img.icon_id + '">');
		for(var i = 0; i< data.label_img.length; i++){
			$(".img-server").prepend('<div><img src="' + data.label_img[i].link + '" class="img-circle" data-iconID="' + data.label_img[i].icon_id + '"><i class="icon-remove-circle dele" position="absolute"></div>');
		};
		$(".per-session .relative-goods").eq(0).append(doT.template(self_tpl.editGoodsTpl)(data.band_goods)).closest('.selected-goods').show();
		$(".form-horizontal .parent-table").css("display","table");
	};
